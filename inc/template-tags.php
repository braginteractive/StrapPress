...
function strappress_posted_on() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && strappress_categorized_blog() ) {
			printf( '<span class="cat-links"><i class="fa fa-folder" aria-hidden="true"></i> ' . esc_html__( '%1$s', 'strappress' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
	}
...
function strappress_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $tags_list ) {
			printf( '<span class="tags-links"><i class="fa fa-tags" aria-hidden="true"></i> ' . esc_html__( '%1$s', 'strappress' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
